...
pair.logL <-	function( par, Y.fam, X.fam,  Y.proband, X.proband, Y.R, X.R, Y.S,
  ...
  LAM12.R <-  lapply(1:nr, function(j) sapply(1:length(R.fR[[j]]), function(i) lam12[lam03$Year.f==R.fR[[j]][i] & lam03$Age.f==A.fR[[j]][i], ]$rate))

  if(!is.null(cut.S)){
    ns <- length(cut.S)
    LAM03.S <-  lapply(1:ns, function(j) sapply(1:length(R.fS[[j]]), function(i) lam03[lam03$Year.f==R.fS[[j]][i] & lam03$Age.f==A.fS[[j]][i], ]$rate))
    LAM12.S <-  lapply(1:ns, function(j) sapply(1:length(R.fS[[j]]), function(i) lam12[lam03$Year.f==R.fS[[j]][i] & lam03$Age.f==A.fS[[j]][i], ]$rate))
  }
...
loglikR_pch_R <- function(par, cut_F, Y_R, X_R,  R.fR, A.fR, cutR, fgau){
  ...
  LAM12.R[[1]] <- sapply(1:length(R.fR), function(i) lam12[lam03$Year.f==R.fR[i] & lam03$Age.f==A.fR[i], ]$rate)
...
loglikS_pch_R <- function(par, cut_F, Y_S, X_S,  R.fS, A.fS, cutS, fgau){
  ...
  LAM12.S[[1]] <- sapply(1:length(R.fS), function(i) lam12[lam03$Year.f==R.fS[i] & lam03$Age.f==A.fS[i], ]$rate)
