...
/*NumericVector ppexp (NumericVector q, NumericVector rate, NumericVector t, bool lower, bool logp)
{
  int i;
  int mi;
  int nt = t.size();
  int nq = q.size();
  LogicalVector tmp(q.size());
  IntegerVector tmp1(q.size());
  IntegerVector idx(q.size());
  NumericVector ret(q.size());
  NumericVector tmpdt1(t.size()-1), tmpdt2(t.size()-1);
  NumericVector dt(t.size()-1), pe(t.size()-1), tt(t.size()-1);
  NumericVector cp(t.size()), rtmp(t.size());
  NumericVector newcp(t.size()-1);
  NumericVector t1(nt), t2(nt);
  tmp = (q<0);
  for(i=0; i<nq; i++) {
    if(tmp[i]==1.0) q[i] = 0;
    tmp1 = (q[i] >= t);
    idx[i] = sum(tmp1)-1;
    ret[i] = R::pexp(q[i] - t[idx[i]], 1/rate[idx[i]], 1.0, 0.0);
  }

  if(t.size()>1){
  if(nt > max(idx)) mi = max(idx);
  else mi = nt;
  ...
  dt =t1- t2;
  rate.erase(mi);
  for(i=0; i<(nt-1); i++) pe[i] = R::pexp(dt[i], 1/rate[i], 1.0, 0.0);

  tt = cumprod3(1 - pe);
  cp[0] = 1;
  for(i=0; i<(nt-1); i++) cp[i+1] = tt[i];

  newcp[0] = 0;
  for(i=0; i<(nt-1); i++) newcp[i+1] = pe[i]*cp[i];
  newcp = cumsum3(newcp);

  for(i=0; i<idx.size(); i++) ret[i] = newcp[idx[i]] + ret[i]*cp[idx[i]];
  }
...
NumericVector dpexp(NumericVector q, NumericVector rate, NumericVector t, bool logp)
{
  int i;
  int nt = t.size();
  int nq = q.size();
  ...
  for(i=0; i<nq; i++) {
    ...
    tmp1 = (q[i] >= t);
    idx[i] = sum(tmp1)-1;
    ret[i] = R::dexp(q[i] - t[idx[i]], 1/rate[idx[i]], logp);
  }
  ...
  dt =t1- t2;
  if(logp == 1.0){
    rate.erase(rate.size());
    for(i=0; i<(nt-1); i++) pe[i] = R::pexp(dt[i], 1/rate[i], 0.0, 1.0);
    tt = cumsum3(pe);
    cp[0] = 0;
    for(i=0; i<(nt-1); i++) cp[i+1] = tt[i];
    for(i=0; i<idx.size(); i++) ret[i] = cp[idx[i]] + ret[i];
  }else{
    rate.erase(rate.size());
    for(i=0; i<(nt-1); i++) pe[i] = R::pexp(dt[i], 1/rate[i], 0.0, 0.0);
    tt = cumprod3(pe);
    cp[0] = 1;
    for(i=0; i<(nt-1); i++) cp[i+1] = tt[i];
    for(i=0; i<idx.size(); i++) ret[i] = cp[idx[i]]*ret[i];
